...
def _make_tracker(output_dir: Path) -> Optional[object]:
    """
    Create a codecarbon EmissionsTracker that writes its own CSV log to
    output_dir.  Returns None if codecarbon is unavailable.
    """
...
def _track_evaluation(tracker) -> Tuple[Optional[float], Optional[float]]:
    """
    Stop a running tracker and return (kg_co2, energy_kwh).
    Returns (None, None) if tracker is None or measurement failed.
    """
    if tracker is None:
        return None, None
    try:
    ...
    for i in range(n_samples):
        ...
        try:
            ...
            flat = build_flat_result(
                i, hyperparams, cv_results, eval_cfg,
                success=True,
                kg_co2=kg_co2,
                energy_kwh=energy_kwh,
            )
