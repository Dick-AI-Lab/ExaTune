#!/usr/bin/env python3
"""
ExaTune Hyperband Search Runner
================================
Runs Hyperband hyperparameter optimization from an ExaTune YAML config and saves
...
-----
    ...
    python run_optuna_local.py --config iris_classification.yaml --n-trials 100 --output-dir ./results/hyperband/iris
...
How it works
------------
Hyperband is a bandit-based early stopping algorithm. CV is run fold-by-fold, and
after each fold the running mean score is reported to Optuna's HyperbandPruner.
Trials that look unpromising after early folds are pruned before completing all folds,
saving compute. Only trials that survive all folds are saved as successful results.

The sampler controls which hyperparameter configurations are suggested:
...
Hyperband parameters:
  --min-resource   Minimum folds before pruning can occur (default: 1)
  --max-resource   Maximum folds = cv_folds from YAML (set automatically)
  --reduction-factor  Bracket reduction factor η (default: 3)
...
# ---------------------------------------------------------------------------
# Cross-validation (fold-by-fold for Hyperband pruning)
# ---------------------------------------------------------------------------
...
def run_cv_with_pruning(
    ...
    """
    Run CV fold-by-fold, reporting the running mean score to Optuna after each
    fold so HyperbandPruner can prune unpromising trials early.

    Returns a cv_results dict compatible with build_flat_result, or raises
    optuna.exceptions.TrialPruned if the trial is pruned mid-way.
    """
    import optuna
    ...
    fold_scores      = {f"test_{primary}": [], f"train_{primary}": []}
    fold_scores.update({f"test_{m}": [] for m in additional})
    fold_scores.update({f"train_{m}": [] for m in additional})
    fit_times        = []
    score_times      = []

    all_metrics = [primary] + additional
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X, y)):
        ...
        for metric in all_metrics:
            ...
            test_score  = scorer(model, X_val,   y_val)
            train_score = scorer(model, X_train, y_train)
            fold_scores[f"test_{metric}"].append(test_score)
            fold_scores[f"train_{metric}"].append(train_score)
        score_times.append(time.time() - t_score)

        # Report running mean of primary metric to Optuna after each fold
        running_mean = float(np.mean(fold_scores[f"test_{primary}"]))
        trial.report(running_mean, step=fold_idx)

        # Let HyperbandPruner decide whether to prune
        if trial.should_prune():
            raise optuna.exceptions.TrialPruned(
                f"Pruned after fold {fold_idx + 1}/{cv_folds} "
                f"(running mean={running_mean:.4f})"
            )

    # Build a cv_results dict matching sklearn's cross_validate output
    cv_results = {"fit_time": np.array(fit_times), "score_time": np.array(score_times)}
    for key, vals in fold_scores.items():
        cv_results[key] = np.array(vals)
...
def run_optuna_search(
    ...
    print("=" * 65)
    print("  ExaTune Hyperband Search")
    print("=" * 65)
    ...
    exp_name    = exp_cfg.get("name", "hyperband_search")
    max_resource = eval_cfg.get("cv_folds", 5)  # max folds = total CV folds

    if output_dir is None:
        base = Path(exp_cfg.get("output_dir", f"./results/hyperband/{exp_name}"))
        output_dir = base
    ...
    print(f"  min_resource    : {min_resource} fold(s)")
    print(f"  max_resource    : {max_resource} fold(s)  [= cv_folds from YAML]")
    ...
    print("\n[2/3] Setting up Optuna study with HyperbandPruner...")
    ...
    print(f"\n[3/3] Running {n_trials} trials (with early fold stopping)...")
    ...
    def objective(trial):
        ...
        try:
            cv_results = run_cv_with_pruning(
                trial, model_cfg, hyperparams, X, y, eval_cfg, random_seed
            )
            ...
            # Record how many folds were actually completed
            flat["folds_completed"] = len(cv_results["fit_time"])
            ...
            n_pruned += 1
            # Save pruned trial with partial results if any folds completed
            flat = build_flat_result(
                trial.number, hyperparams, {}, eval_cfg,
                success=False, error_message="pruned by HyperbandPruner",
                kg_co2=None, energy_kwh=None,
            )
            flat["folds_completed"] = 0
            rows.append(flat)
            if verbose:
                print(f"  [{trial.number+1:>{len(str(n_trials))}}/{n_trials}] "
                      f"PRUNED  params={hyperparams}")
            raise
    ...
    n_completed = len([r for r in rows if r.get("success")])
    print(f"\n  Completed {n_completed} full trials, {n_pruned} pruned, "
          f"in {elapsed_total:.1f}s ({elapsed_total/max(len(rows),1):.2f}s/trial)")
    ...
    # Convergence summary (successful trials only)
    successful = df[df["success"] == True].copy()
    if not successful.empty:
        successful = successful.reset_index(drop=True)
        successful["best_so_far"] = successful["mean_score"].cummax()

        print("\n  Convergence (successful trials only):")
        checkpoints = [int(len(successful) * f) - 1 for f in [0.1, 0.25, 0.5, 0.75, 1.0]]
    ...
    if _CODECARBON_AVAILABLE and "kg_co2" in df.columns:
        ...
        print(f"    Per trial   : {total_co2/n_trials*1e6:.2f} µg CO₂eq avg")

    print(f"\n  Pruning summary: {n_pruned}/{n_trials} trials pruned early "
          f"({100*n_pruned/max(n_trials,1):.1f}% compute saved)")
...
def main():
    parser = argparse.ArgumentParser(
        description="ExaTune Hyperband hyperparameter search — compatible with all ExaTune visualizations",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Requires:  pip install optuna

Examples:
  # 100 trials with default TPE sampler + Hyperband pruning
  python run_optuna_local.py --config iris_classification.yaml --n-trials 100

  # Use Random sampler (useful as a baseline)
  python run_optuna_local.py --config iris_classification.yaml --n-trials 100 --sampler Random

  # Tune Hyperband aggressiveness (higher reduction-factor = more aggressive pruning)
  python run_optuna_local.py --config iris_classification.yaml --n-trials 100 --reduction-factor 4

  # Require at least 2 folds before pruning is allowed
  python run_optuna_local.py --config iris_classification.yaml --n-trials 100 --min-resource 2

  # Verbose output (per-trial results including fold count)
...
Hyperband parameters:
  --min-resource      Minimum folds before pruning kicks in (default: 1)
  --reduction-factor  Bracket reduction factor η (default: 3, higher = more aggressive)
  max_resource is set automatically from cv_folds in your YAML

Visualizing results:
  df = pd.read_parquet("./results/hyperband/.../results.parquet")
  # Only use successful trials for analysis:
  df = df[df['success'] == True]
    ...
    parser.add_argument("--min-resource", type=int, default=1,
                        help="Minimum folds before Hyperband can prune (default: 1)")
    parser.add_argument("--reduction-factor", type=int, default=3,
                        help="Hyperband reduction factor η (default: 3)")
    ...
    parser.add_argument("--verbose", "-v", action="store_true",
                        help="Print per-trial results including fold count and pruning")
