...
def infer_metrics(parquet_path: Path) -> list:
    ...
    # Mirror the column flattening that complete_analysis.py does
    ...
    # Also expand nested additional_metrics dicts (pre-flatten parquets store them this way)
    if "additional_metrics" in df.columns:
        try:
            nested = pd.json_normalize(df["additional_metrics"].dropna())
            nested.columns = [c.replace(".", "_") for c in nested.columns]
            df = pd.concat([df.drop(columns=["additional_metrics"]), nested], axis=1)
        except Exception:
            pass
